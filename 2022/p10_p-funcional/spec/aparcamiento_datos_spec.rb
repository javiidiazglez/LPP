RSpec.describe Aparcamiento do
  ...
  context "Representación de los Datos de un aparcamiento - Aparcamiento::Datos" do
    ...
    @datos_1 = Aparcamiento::Datos.new(5, 5, 1, "AparcamientoElDios", :mixto, :autobuses, ['libre'], [[3.0,4.0,5.0]], 1.2, 0, 1.0, @conjunto_vehiculos_1)
    @datos_2 = Aparcamiento::Datos.new(4, 10, 2, "AparcamientoJavi", :cubierto, :coches, ['libre','ocupada'], [[3.0,4.0,5.0], [5.0,6.0,7.0]], 2.5, 1, 1.25, @conjunto_vehiculos_2)
    @datos_3 = Aparcamiento::Datos.new(6, 10, 3, "AparcamientoBajo", :aire_libre, :bicicletas, ['ocupada','libre','ocupada'],[[3.0,4.0,5.0], [5.0,6.0,7.0], [8.0,9.0,10.0]], 1.0, 2, 2.0, @conjunto_vehiculos_3)
    @datos_4 = Aparcamiento::Datos.new(8, 11, 4, "AparcamientoSolar", :aire_libre, :motos, ['ocupada','ocupada','ocupada','libre'], [[3.0,4.0,5.0], [5.0,6.0,7.0], [8.0,9.0,10.0], [11.0,12.0,13.0]], 0.0, 3, 1.0, @conjunto_vehiculos_3)
    @datos_5 = Aparcamiento::Datos.new(9, 12, 3, "AparcamientoVago", :aire_libre, :bicicletas, ['ocupado', 'ocupado', 'ocupado'], [[3.0,4.0,5.0,1], [5,6,7,2], [8,9,10,1], [11,12,13,2],[11,12,13,14]], 40.0, 3, 1.0, @conjunto_vehiculos_3)  
    @datos_6 = Aparcamiento::Datos.new(5, 5, 6, "AparcamientoElDios", :mixto, :autobuses, ['libre', 'libre', 'libre'], [[3.0,4.0,5.0],[5.0,6.0,7.0], [8.0,9.0,10.0]], 21.0, 3, 0.10, @conjunto_vehiculos_3)
    @datos_7 = Aparcamiento::Datos.new(5, 5, 7, "AparcamientoElDios", :mixto, :autobuses, ['libre', 'libre', 'libre'], [[3.0,4.0,5.0],[5.0,6.0,7.0], [8.0,9.0,10.0]], 40.0, 3, 0.09, @conjunto_vehiculos_3)
    ...
    @conjunto_aparcamientos_1 = [@datos_1, @datos_2, @datos_3, @datos_6] # datos_6
    @conjunto_aparcamientos_2 = [@datos_1, @datos_2, @datos_6, @datos_7] # datos_7
    @conjunto_aparcamientos_3 = [@datos_1, @datos_2, @datos_3] # datos_3
  end
    context "Atributos de la clase Aparcamiento::Datos" do
      ...
      it "Tiene un atributo para representar el conjunto de plazas libres y ocupadas" do
        ...
        expect(@datos_3.plazas_ocupadaslibres).to eq(["ocupada", "libre", "ocupada"])
      ...
      it "Un aparcamiento tiene atributo para representar la plaza de minusvalido" do
        ...
        expect(@datos_1.n_plazas_minusvalido).to eq(0)
        expect(@datos_2.n_plazas_minusvalido).to eq(1)
        expect(@datos_3.n_plazas_minusvalido).to eq(2)
        expect(@datos_4.n_plazas_minusvalido).to eq(3)
        expect(@datos_5.n_plazas_minusvalido).to eq(3)
      end
      ...
      it "Se espera que un estacionamiento cuente con una funcionalidad que permita aparcar un vehículo." do
        expect(@datos_6.aparcar(@vehiculo_4, 3)).to eq "Aparcamiento seleccionado ocupado"
        ...
        expect(@datos_3.aparcar(@vehiculo_3, 1)).to eq "Aparcamiento seleccionado ocupado"
        expect(@datos_6.aparcar(@vehiculo_4, 3)).to eq "Aparcamiento seleccionado ocupado"
        expect(@datos_6.aparcar(@motor_1, 3)).to eq "Aparcamiento seleccionado ocupado"
      ...
      context "Práctica 10. PARADIGMA DE PROGRAMACION FUNCIONAL." do
        ...
        it "Se espera seleccionar los estacionamientos con plazas libres de mayor índice de sostenibilidad del conjunto de aparcamientos" do
          expect(@conjunto_aparcamientos_1.select{|estacionamiento| estacionamiento.numero_plazas_libres > 0}.max).not_to be nil
          expect(@conjunto_aparcamientos_1.select{|estacionamiento| estacionamiento.numero_plazas_libres > 0}.max).to eq @datos_6
          expect(@conjunto_aparcamientos_2.select{|estacionamiento| estacionamiento.numero_plazas_libres > 0}.max).to eq @datos_7
          expect(@conjunto_aparcamientos_3.select{|estacionamiento| estacionamiento.numero_plazas_libres > 0}.max).to eq @datos_1
        end
        ...
        it "Se espera calcular el porcentaje de ocupación de cada uno de ellos del conjunto de aparcamientos" do
          expect(@conjunto_aparcamientos_1.collect{|estacionamiento| ((Float(estacionamiento.numero_plazas - estacionamiento.numero_plazas_libres) / estacionamiento.numero_plazas)* 100).round(2) }).not_to be nil
          expect(@conjunto_aparcamientos_1.collect{|estacionamiento| ((Float(estacionamiento.numero_plazas - estacionamiento.numero_plazas_libres) / estacionamiento.numero_plazas)* 100).round(2) }).to eq [0.0, 50.0, 66.67, 0.0]
          expect(@conjunto_aparcamientos_2.collect{|estacionamiento| ((Float(estacionamiento.numero_plazas - estacionamiento.numero_plazas_libres) / estacionamiento.numero_plazas)* 100).round(2) }).to eq [0.0, 50.0, 0.0, 0.0]
          expect(@conjunto_aparcamientos_3.collect{|estacionamiento| ((Float(estacionamiento.numero_plazas - estacionamiento.numero_plazas_libres) / estacionamiento.numero_plazas)* 100).round(2) }).to eq [0.0, 50.0, 66.67]
        end
        it "Se espera calcular el porcentaje de plazas de minusválidos libres de cada uno del conjunto de aparcamientos" do
          expect(@conjunto_aparcamientos_1.collect{|estacionamiento| (estacionamiento.n_plazas_minusvalido > 0) ? (Float(Aparcamiento::Funcionalidades::libre_minusvalidos(estacionamiento)) / estacionamiento.n_plazas_minusvalido * 100).round(2) : 0.0}).not_to be nil
          expect(@conjunto_aparcamientos_1.collect{|estacionamiento| (estacionamiento.n_plazas_minusvalido > 0) ? (Float(Aparcamiento::Funcionalidades::libre_minusvalidos(estacionamiento)) / estacionamiento.n_plazas_minusvalido * 100).round(2) : 0.0}).to eq [0.0, 0.0, 50.0, 66.67]
          expect(@conjunto_aparcamientos_2.collect{|estacionamiento| (estacionamiento.n_plazas_minusvalido > 0) ? (Float(Aparcamiento::Funcionalidades::libre_minusvalidos(estacionamiento)) / estacionamiento.n_plazas_minusvalido * 100).round(2) : 0.0}).to eq [0.0, 0.0, 66.67, 66.67]
          expect(@conjunto_aparcamientos_3.collect{|estacionamiento| (estacionamiento.n_plazas_minusvalido > 0) ? (Float(Aparcamiento::Funcionalidades::libre_minusvalidos(estacionamiento)) / estacionamiento.n_plazas_minusvalido * 100).round(2) : 0.0}).to eq [0.0, 0.0, 50.0]
        end
        ...
        it "Se espera realizan comparaciones de datos usando modulo comparable" do 
          expect(@datos_1 > @datos_2).to eq(false) 
          expect(@datos_3 > @datos_1).to eq(false) 
          expect(@datos_4 < @datos_1).to eq(false) 
          expect(@datos_6 >= @datos_1).to eq(true) 
          expect(@datos_4 <= @datos_1).to eq(true) 
          expect(@datos_4 == @datos_1).to eq(true) 
          expect(@datos_4.between?(@datos_6,@datos_3)).to eq(false)
          expect(@datos_1.between?(@datos_2,@datos_3)).to eq(true)
        end
      end
      it "Tiene un método para obtener una cadena con la información del aparcamiento correctamente formateada" do
        expect(@datos_2.to_s).to eq("Aparcamiento::Datos: => 4, 10, 2, AparcamientoJavi, cubierto, coches, [\"libre\", \"ocupada\"], 2.5, 1, 1.25, Aparcamiento::Vehiculo: => (55) - (1.2) - (3.401) - (3.405) - (3.5671)")
      end
    end
